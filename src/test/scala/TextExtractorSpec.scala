...
class TextExtractorSpec extends AnyFlatSpec {
  ...
  "Extracting spotify tracks" should "retain only spotify track links" in {
    ...
    val result = inputs
      .collect(
        TextExtractor.extractValuesWithPattern(Main.spotifyTrackPattern)
      )
      .compile
      .toList
    ...
    assert(
      result === List(
        "https://open.spotify.com/track/6LxcPUqx6noURdA5qc4BAT",
        "https://open.spotify.com/track/190safjas90fj120"
      )
    )
  }
}
